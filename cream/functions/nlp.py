...
doublesreplace = 'ㅡㅣ,ㅗㅏ,ㅗㅐ,ㅗㅣ,ㅜㅣ,ㅜㅓ,ㅜㅔ,ㅣㅓ'.split(',')
doubles = 'ㅢ,ㅘ,ㅙ,ㅚ,ㅟ,ㅝ,ㅞ,ㅕ'.split(',')
...
def SplitString(string:str) -> list:
    '''
    Split Cho, Jung, JongSung of Korean Word
    '''

    r_lst = []

    for w in split(string):
        if '가'<=w<='힣':
            ch1 = (ord(w) - ord('가'))//588
            ...
            if JONGSUNG_LIST[ch3] != ' ':
                r_lst = r_lst + [CHOSUNG_LIST[ch1]] + split(doublevowel(JUNGSUNG_LIST[ch2])) + [JONGSUNG_LIST[ch3]]
            else:
                r_lst = r_lst + [CHOSUNG_LIST[ch1]] + split(doublevowel(JUNGSUNG_LIST[ch2]))
        else:
            r_lst.append(w)
    return r_lst
